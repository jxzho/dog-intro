      ...
      <ul class="project-list" ref="container">
        <li class="project-item" v-for="(item, index) of projects" :key="index">
          ...
          <div class="title-wrapper">
            ...
            <img
              class="preview"
              src="https://junxio-static.oss-cn-shenzhen.aliyuncs.com/static/info/eye.svg"
          ...
          <button :class="['btn', { up: index === showHideIndex }]" @click="handleChangeSHIndex(index)">
            <svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="30" height="30">
              <path
                d="M972.69 286.413L529.478 757.491c-1.327 1.327-3.096 1.769-3.981 1.769s-2.654 0-3.981-1.327L49.995 314.721c-11.943-11.5-30.963-10.616-42.021 1.327s-10.616 30.963 1.327 42.021l471.078 443.212c12.827 11.943 29.194 18.135 46.887 17.693 17.251-.442 33.617-7.962 45.56-20.347l443.212-471.521c5.308-5.75 7.962-13.27 7.961-20.347 0-7.962-3.096-15.924-9.289-21.674-11.943-11.5-30.963-10.616-42.021 1.327z"
              />
            </svg>
          </button>
          ...
          <transition>
            <div class="show" v-show="index === showIndex">
              ...
              <a :href="item.link" target="_black"
                ><img class="link" src="https://junxio-static.oss-cn-shenzhen.aliyuncs.com/static/info/link.svg"
              /></a>
              <img
                class="qr-code"
              ...
              <transition name="capture">
                <img class="capture" :src="item.capture" v-show="captureIndex === index" />
              </transition>
            </div>
          </transition>
        </li>
      ...
      projects: [
        {
          title: '交互式简历',
          content:
            '在移动端网页上展示我的简历，展示过程采取电话接听、微信聊天等引领用户了解我，用户可通过一些操作一步步阅读我的简历，是使用原生JS、CSS等辅助库编写的单页应用。',
          detail: [
            '使用单例、发布订阅模式开发。',
            'CSS3，使用animation等属性，animate动画库等控制动画。',
            'HTML5语义标签，页面结构更清晰。',
            '预加载。',
            '部分优化。',
          ],
        ...
        },
        {
          title: 'QQ音乐播放页面',
          content:
            '为了锻炼js正则，自己模仿QQ音乐移动端网页版播放页面，实现通过ajax获取歌词并用正则解析歌词放置页面展示，功能：自动播放歌曲，歌词自动滚动，暂停与续播歌曲。',
          detail: [
            '加载页面适配。',
            'less',
            'ajax获取歌词，js正则解析歌词。',
        ...
        },
        {
          title: '去哪儿网',
          content:
            '学习完vue后利用vue-cli、vue-router、vuex做的小项目，目的是为了利用所学的vue知识实现去哪儿网部分功能,例如：首页与详情页展示、星星评分、切换/搜索城市、图片相册的轮播功能等等，是一次对vue基本知识的实践,通过项目查清弥补自己vue知识的缺漏',
          detail: [
            'vue-router对页面路由，首页组件缓存',
            'swiper轮播首页广告以及相册展示。',
            'localstorage缓存数据，保证下次打开仍显示当前城市。',
            'iscoll解决移动端滚动功能。',
            'ajax获取数据，数据存放在json，后台express处理请求。',
            'less编写样式,提高效率',
      ...
      .project-item {
        ...
        .detail {
          ...
          &.act {
            ...
            overflow: visible;
            opacity: 1;
            transition: max-height 0.6s ease-in-out, opacity 0.6s 0.2s ease-in-out;
          }
